...
public class RandomNumber<T extends Number & Comparable<T>> {
    @Getter private T lowerBound;
    @Getter private T upperBound;
    ...
    public Long generateLong() {
        if(lowerBound.equals(upperBound)) return lowerBound.longValue();
        return ThreadLocalRandom.current().nextLong(lowerBound.longValue(), upperBound.longValue()+1);
    }

    public Double generateDouble() {
        if(lowerBound.equals(upperBound)) return lowerBound.doubleValue();
