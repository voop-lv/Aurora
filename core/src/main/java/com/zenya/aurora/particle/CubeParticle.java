...
public class CubeParticle extends SimpleParticleTask {
    ...
    public CubeParticle(Player player, Location[] locs, ParticleFile particleFile) {
        ...
        BukkitTask drawShape = AuroraAPI.getAPI().getParticleFactory().createCube(locs[locIndex], length.generateDouble(), rate.generateDouble(), update.generateInt(), duration.generateLong(), rotationAngle.generateDouble(), rotationAxis, display);
